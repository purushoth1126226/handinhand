...
<x-admin.layouts.adminindexnav can="ttest" name=drugexpiryreport title="{{__('label.sidenav_report_expiry')}}" button='disable' gate='true' />
<main class="w-full flex-grow p-3">
       ...
       <div class="w-full py-3">
         ...
         <div class="inline-block mr-2 mt-2">
            <a href=""  id="drugexpiryreportcsv" target="_blank" class="focus:outline-none text-white text-sm py-2.5 px-5 rounded-md bg-blue-500 hover:bg-blue-600 hover:shadow-lg">{{ __('label.patientenrollmentreport_csv_index')}}</a>
         </div>
           <div class="inline-block mr-2 mt-2">
       ...
       <div class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
          <table id="ajaxdrugexpiry" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
      ...
      <tr>
         ...
         <th>{{__('label.inward_expirydate_create')}}</th>
...
$(document).ready(function() {
    ...
    function load_data(from_date = '', to_date = '') {
        ...
        $('#ajaxdrugexpiry').DataTable({
        processing: true,
        serverSide: true,
        responsive: true,
        "ajax": {
            "url": '{!! route('report.ajaxdrugexpiryreport') !!}',
    ...
    $('#filter').click(function() {
        ...
        if (from_date != '' && to_date != '') {
            $('#ajaxdrugexpiry').DataTable().destroy();
    ...
    $('#drugexpiryreportcsv').click(function() {
        var from_date = $('#from_date').val();
        var to_date = $('#to_date').val();
        if (from_date != '' && to_date != '') {
            var url = 'drugexpiryreportcsv/'+from_date+'/'+to_date;
            $("#drugexpiryreportcsv").attr("href", url)
            $('#ajaxdrugexpiry').DataTable().destroy();
            load_data(from_date, to_date);
        } else {
            alert('Both Date is required');

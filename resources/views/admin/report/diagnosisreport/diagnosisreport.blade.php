...
<x-admin.layouts.adminindexnav can="ttest" name=diagnosisreport title="DIAGNOSIS REPORT" button='disable' gate='true' />
 ...
 <center>
       <div class="w-full py-3">
        ...
        <div class="inline-block mr-2 mt-2">
            {{ Form::select('diagnosis_id', $diagnosis, null, array('id'=>'diagnosis_id', 'class' => 'form-input rounded block w-full p-2 focus:bg-white',  'readonly' => 'readonly')) }}
           </div>
         ...
         <div class="inline-block mr-2 mt-2">
            <a href=""  id="diagnosisreportcsv" target="_blank" class="focus:outline-none text-white text-sm py-2.5 px-5 rounded-md bg-blue-500 hover:bg-blue-600 hover:shadow-lg">CSV</a>
         </div>
           <div class="inline-block mr-2 mt-2">
       ...
       <div class="p-8 mt-6 lg:mt-0 rounded shadow bg-white">
          <table id="ajaxdiagnosis" class="stripe hover" style="width:100%; padding-top: 1em;  padding-bottom: 1em;">
             <thead class="bg-blue-600 text-white">
                ...
                <th>{{__('label.patientenrollment_sno_index')}}</th>
                    ...
                    <th>{{__('label.diagnosis_name_index')}}</th>
...
$(document).ready(function() {
    ...
    function load_data(from_date = '', to_date = '', diagnosis_id) {

        $('#ajaxdiagnosis').DataTable({
        processing: true,
        serverSide: true,
        responsive: true,
        "ajax": {
            "url": '{!! route('report.ajaxdiagnosisreport') !!}',
            'headers': {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
            data:{from_date:from_date, to_date:to_date, diagnosis_id},
            "type": "POST"
        },
        columns: [
    ...
    $('#filter').click(function() {
        ...
        if (from_date != '' && to_date != '') {
            $('#ajaxdiagnosis').DataTable().destroy();
            load_data(from_date, to_date, diagnosis_id);
        } else {
    ...
    $('#diagnosisreportcsv').click(function() {
        var from_date = $('#from_date').val();
        var to_date = $('#to_date').val();
        var diagnosis_id = $('#diagnosis_id').val();
        if (from_date != '' && to_date != '') {
            var url = 'diagnosisreportcsv/'+from_date+'/'+to_date+'/'+diagnosis_id;
            $("#diagnosisreportcsv").attr("href", url)
            $('#ajaxdiagnosis').DataTable().destroy();
            load_data(from_date, to_date, diagnosis_id);
        } else {
            alert('Both Date is required');
